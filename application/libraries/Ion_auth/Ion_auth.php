...
class User extends Ion_auth {
    ...
    // Opciones de usuario no identificado
    private $logedIn  = false;
    private $username = 'Invitado';
    private $first_name = 'Invitado';
    private $last_name  = '';
    private $email    = '';
    private $id       = null;
	private $permisos = null;
	private $roles    = array();
	private $aclPerm  = null;

	/**
	 * Por defecto, el usuario Invitado ya está seteado
	 */
    public function __construct() {

        parent::__construct();

        $this->logedIn = $this->logged_in();

        if ( $this->logedIn ) {
            $this->setUser();
        } else {
			$this->permisos = (object) array(
				'create' => false
			  , 'read'	 => false
			  , 'update' => false
			  , 'delete' => false
		...
		// Seteamos sus permisos:
		$this->load->model('acl_model', 'Acl');
		$rolId = ( empty( $this->roles ) ? 0 : $this->roles[0]->id );
		$this->aclPerm = $this->Acl->getPermisos( $rolId, $this->id );
    }

    /**
     * Por defecto, cogemos sólo el nombre de usuario y email
     */
    private function setUser() {
        $campos = 'id, first_name, last_name, username, email';

//        $user = $this->ci->ion_auth_model->select( $campos )->user()->row();
//
//        foreach ( $user as $var => $value ) {
//            $this->{$var} = $value;
//        }
//
//		$this->roles = $this->ci->ion_auth_model->get_users_groups( $this->id )->result();
    }

	/**
	 * Devuelve los permisos de la ACL
	 * @return type
	 */
	public function checkAclPerm( $zone, $permission ) {
		try {
			if ( ! isset( $this->aclPerm->{$zone}->{$permission} ) ) {
				throw new Exception('No existe el permiso solicitado: ' . $permission . ' para la zona ' . $zone);
			}

			return $this->aclPerm->{$zone}->{$permission};
		} catch ( Exception $e ) {
			throw new Exception( '¡PERMISO NO ENCONTRADO!: ' . $e->getMessage() );
		}
	}

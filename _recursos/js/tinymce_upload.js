...
var jbImagesDialog = {
	...
	uploadFinish : function(result) {
		...
		else
		{
			document.getElementById("upload_in_progress").style.display = 'none';
			document.getElementById("upload_infobar").style.display = 'block';
			document.getElementById("upload_infobar").innerHTML = 'Almacenado correctamente';
			
			var w = this.getWin();
			tinymce = w.tinymce;
			
      tinymce.EditorManager.activeEditor.insertContent('<span><img class="thumbnail img-responsive" src="' + result.filename +'"></span>');
